...
Because graphs can be of different sizes (varying numbers of nodes and edges), we cannot batch graphs the traditional way by stacking them in a list. Instead, we batch the graphs by **constructing a single, large graph** consisting of disjoint subgraphs within it.
